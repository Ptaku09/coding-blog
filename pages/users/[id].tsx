...
const User: ({ userData }: { userData: User }) => JSX.Element = ({ userData }: { userData: User }) => {
  ...
  return (
    <div className="w-screen h-auto min-h-screen pb-12 bg-white dark:bg-dark">
      {userData ? (
        <>
          <section className="flex flex-col items-start justify-start">
            ...
            <div className="w-screen h-auto px-5 bg-white -translate-y-4 rounded-t-xl">
              <div className="w-32 h-32 relative -translate-y-16 rounded-full border-4 border-white dark:border-dark overflow-hidden">
                <Image src={userData.image} objectFit="cover" layout="fill" alt="avatar" />
              </div>
              <div className="pb-8 -translate-y-12 flex items-start justify-start flex-col font-raleway border-b-[1px] dark:border-dark">
                <h2 className="font-bebas text-4xl">{userData.username}</h2>
                <h6 className="font-raleway font-bold text-xs text-purple-600">{userData.name.toUpperCase()}</h6>
